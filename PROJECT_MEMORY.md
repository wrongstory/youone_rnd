...
- `OD-042` 최초 연구소장 bootstrap 권장 ceremony는 `docs/identity-bootstrap.md`에 분리했다. `OD-043` 공개 가입 남용방지는 아직 승인 전이며 `docs/registration-abuse-policy.md`의 email `3/24h`, HMAC network `10/h·30/24h`, global `100/h·500/24h`, always-on Turnstile 권장안이 승인되기 전 공개 endpoint는 fail-closed한다.
