...
`IMPLEMENTATION_ACTIVE` (P0 `M00`~`M16`과 Release Gate #36 R01~R06 코드 계약이 `dev`에 병합 완료. 실제 사용자 정책 승인, Staging 27개 증거와 `dev → main` 승격은 아직 없다. P1 권장 범위·로드맵은 승인됐고 이슈 #48에서 source/ERD/권한/상태/ADR 설계 Gate를 진행하지만 P0 릴리즈와 별도 P1 Development Gate 전에는 제품 코드를 시작하지 않는다).
...
P1 계획 `DELIVERY-PLAN-P1-V0.1`과 확정 범위 `P1-SCOPE-V1.0`은 GitHub 이슈 #39 및 PR #40으로 승인·병합됐다. P1은 BOM, 연구장비·교정, 안전관리 확장, 연구수당, 권한필터 통합검색을 권장 깊이와 순서로 진행한다. 이슈 #48/PR #49는 source delta, domain/ERD, 권한, 상태머신, ADR-011과 workstream/merge order의 설계 검토를 소유한다. 보강 검토는 sealed 반려본 `RETURNED`/successor draft, Equipment serviceability·checkout·usage 축 분리, Calibration/Search policy approval lifecycle, Search index/purge failure 분리를 요구했다. 초기 검색은 6개 source군 metadata-only, Vendor BOM은 exact Project+Contract+assignment, 교정 예외는 versioned policy/exact scope/Approval, 수당 participant는 canonical 공식 approval authority, offline은 draft capture-only로 기록했고 OD-024는 본 연구수당 규정 제5조 self-reference로 정정한다. P0 Release Gate `#36` 완료와 `dev → main` P0 승격, 해당 설계 검토 및 별도 Development Gate 승인 전에는 P1 제품 코드나 migration을 만들지 않는다.
