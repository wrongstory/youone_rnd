...
`IMPLEMENTATION_ACTIVE` (P0 `M00`~`M16`과 Release Gate #36 R01~R06 코드 계약이 `dev`에 병합 완료. `OD-019`, `OD-035`, `OD-036`과 `OD-039` 정책값, Primary/Recovery/Vercel 및 monitoring/evidence/Google Drive backup stable ID는 사용자 승인됐다. 2026-08-25 Supabase는 당분간 Free Plan으로 유지하고 Pro 전용 session control은 매 요청 Identity Resolver와 application-owned DeviceTrust/StepUpGrant, 외부 운영 Worker로 대체하기로 했다. 실제 actor binding, effective approval snapshot, 외부 Worker 실증, Staging 27개 증거와 `dev → main` 승격은 아직 없다. P1 범위·설계 Gate는 승인됐지만 P0 릴리즈와 별도 P1 Development Gate 전에는 제품 코드를 시작하지 않는다).
...
Release Gate #36 R02 request Auth는 `@supabase/supabase-js`를 request adapter 안에만 두고 publishable key, session persistence/refresh 비활성화, explicit-token `getUser + getClaims`, bounded health probe를 적용한다. `ActorContextSource`는 verified subject와 provider `session_id`를 함께 받아 별도 최소권한 `youone_identity_resolver` pool에서 `auth.sessions` exact subject/session 활성행을 확인한 뒤에만 DB identity snapshot을 반환한다. Auth readiness는 provider와 resolver capability가 모두 성공해야 한다. Supabase ban은 기존 session revoke가 아니고 delete는 비활성화와 다르다. 사용자는 global sign-out + exact session 요청별 검사, 최대 잔존 60분, retry 3회, reconciliation 15분 및 공급자 잔여위험을 승인했으며, 실제 actor-bound snapshot과 Staging 증거 전까지 release blocker는 열린다.

Issue `#58` 1차 운영 Auth slice는 Supabase password login, TOTP enroll/verify, refresh, generic recovery request, global sign-out 호출과 trusted current-session API를 provider adapter/Web composition 경계로 추가한다. Production token·refresh·factor context는 `Secure`/`HttpOnly`/`SameSite=Strict`/`__Host-` cookie에만 두고 mutation은 same-origin+CSRF를 요구하며 response/log schema에서 credential과 provider 원문을 배제한다. AAL1은 업무 ActorContext를 만들 수 없다. 실제 Staging 설정, 분산 rate limit, recovery confirmation, device trust/step-up이 남아 있어 B01과 #58은 계속 열린다. 상세 gap은 `docs/p0-operational-backend.md`가 기록한다.
...
Issue `#58` B01 abuse-prevention slice는 login/recovery의 normalized identifier와 로그인 성공 시 발급한 256-bit HttpOnly stable limiter-subject를 분리한다. limiter-subject cookie는 `nonce.signature`로 저장하고 deployment HMAC secret 및 `YOUONE_AUTH_RATE_SUBJECT_V1` context로 인증한다. logout/TOTP enroll·verify/refresh는 token rotation과 무관한 verified nonce를 공유하며 cookie가 없거나 변조되면 Provider와 limiter 호출 전 fail-closed한다. PostgreSQL은 subject+deployment-global fixed-window counter를 원자 소비하고 consume/result Audit 모두 exact policy UUID를 참조한다. canonical V1 hash가 정확히 6개 rule과 일치하고, distinct active `ADMIN_SECURITY` agreement + `POSITION_LAB_DIRECTOR` approval action을 가진 completed ApprovalInstance가 결합된 경우만 정책을 활성화한다. 수치·실제 2인 actor·HMAC secret은 임의 seed하지 않는다.

Issue `#65`는 P0 DeviceTrust·ActivationContext 최소 vertical slice를 구현한다. 일반 업무 `ActorContext`와 분리된 branded PENDING-only context는 exact configured Supabase user/session, live `auth.sessions`, TOTP `aal2`, immutable registration/`OD-042` evidence만 받으며 DeviceTrust 등록·검증·readiness 외 Role/Scope/업무 기능을 갖지 않는다. 전용 `youone_activation` NOLOGIN/NOINHERIT/NOBYPASSRLS capability와 FORCE RLS table/RPC, no-seed canonical-hash policy, exact UserAccount/session/evidence/policy HMAC binding, 2단계 nonce cookie와 replay 차단을 구현했다. DeviceTrust 활성화와 UserAccount 활성화는 별도 transaction이며 후자는 invite/password, TOTP, ACTIVE trust, assignment 또는 VendorMembership을 모두 재검증한다. Hosted Supabase에는 아직 적용하지 않았고 policy duration/secret/actor/evidence도 seed하지 않았으므로 Staging 활성화는 계속 fail-closed다.

Supabase 연결 감사에서는 `ap-northeast-1`의 ACTIVE_HEALTHY 프로젝트 2개가 확인됐다. 사용자는 2026-08-25 `wrongstory's Project` (`dttwfqzkhjujqkcatyav`)를 `YOUONE_STAGING_PRIMARY`, 기존 `YOUONE_STAGING_RECOVERY` (`jzxhetszlucgutnwidkd`)를 격리 복구 대상으로 확정했다. 이후 Supabase는 당분간 Free Plan으로 유지하기로 최종 변경했다. 업무/Auth 데이터는 0건이고 Recovery는 migration/table이 비어 있다. 같은 날 Primary에 M11~M16, R02/R03/R06 및 B01 3개를 순서대로 적용해 hosted migration 21개가 됐다. 적용 후 public RLS 미적용 table과 `anon`/`authenticated`가 실행할 수 있는 public `SECURITY DEFINER`는 각각 0이며 Security Advisor는 WARN/ERROR 0, policy가 없는 FORCE-RLS table INFO만 남는다.
...
Release Gate #36 R06는 `OD-019`, `OD-035`, `OD-036`의 사용자 승인값을 임의 기본값 없이 versioned·unrevoked policy snapshot과 `created <= approved <= effective <= evaluated` 시간 관계로 검증한다. 각 approval artifact의 `approvedPolicySha256`을 approval 제외 정책 본문 전체의 canonical SHA-256과 exact 비교하므로 action/session/device, recovery/backup/actor/destination, revoke retry/reconciliation 변경에 기존 승인을 재사용할 수 없다. OD-035는 승인된 recovery approver/executor 집합의 무교집합을 강제하고 실제 `RECOVERY_DB_STORAGE` artifact의 actor·정책 hash·RTO 시간뿐 아니라 candidate/promotion commit, `YOUONE_STAGING_PRIMARY → YOUONE_STAGING_RECOVERY`, candidate checkout에서 산출한 migration head를 exact 검증한다. quality·M07~M16·R01~R05·migration·recovery·Staging·PWA/mobile·security의 정확히 27개 artifact를 closed ID 경로에서 직접 읽어 raw SHA-256을 재계산하고, evidence/R05/trusted promotion source commit을 exact 결합하며 R05 canonical digest/readiness/check를 재검증한다. OD-036 runtime은 trusted target resource와 JWT `sub/session_id/issuer` 및 active `auth.sessions`를 결합하고 global sign-out 뒤 exact session 부재를 확인한다. CLI parse/read/validator 오류도 비밀값 없는 `BLOCKED`와 non-zero exit로 수렴한다. 모든 증거를 충족한 상태도 `READY_FOR_RELEASE_PR`일 뿐이며 별도 사용자 승인 없이는 `main`을 갱신하지 않는다.

P1 계획 `DELIVERY-PLAN-P1-V0.1`과 확정 범위 `P1-SCOPE-V1.0`은 GitHub 이슈 #39 및 PR #40으로 승인·병합됐다. P1은 BOM, 연구장비·교정, 안전관리 확장, 연구수당, 권한필터 통합검색을 권장 깊이와 순서로 진행한다. 이슈 #48/PR #49의 source delta, domain/ERD, 권한, 상태머신, ADR-011과 workstream/merge order 설계 Gate도 사용자 검토 후 `dev`에 병합됐다. sealed 반려본 `RETURNED`/successor draft, Equipment serviceability·checkout·usage 축 분리, Calibration/Search policy approval lifecycle, Search index/purge failure 분리를 확정했다. 초기 검색은 6개 source군 metadata-only, Vendor BOM은 exact Project+Contract+assignment, 교정 예외는 versioned policy/exact scope/Approval, 수당 participant는 canonical 공식 approval authority, offline은 draft capture-only로 기록했고 OD-024는 본 연구수당 규정 제5조 self-reference로 정정한다. P0 Release Gate `#36` 완료와 `dev → main` P0 승격 및 별도 P1 Development Gate 승인 전에는 P1 제품 코드나 migration을 만들지 않는다.
...
- `OD-035` 기간값은 RPO 60분/RTO 240분, DB 60분/14일, Storage 60분/30일로 확정됐다. monitoring stable ID 3개와 `OPS_EVIDENCE_PRIVATE_PRIMARY`, 비공유 Google Drive 외부 백업이 승인됐고 전용 비공유 폴더 구조가 생성됐다. 실제 사고대응 담당자와 독립 복구 승인자/실행자 `UserAccount` UUID는 Staging 계정에서 선택해야 하며 Position/Role seed UUID나 preview 사용자를 대신 쓰지 않는다. Drive folder ID와 OAuth 자격증명은 저장소나 증거에 기록하지 않고 운영 secret store에서만 stable ID에 결합한다.
- 인증 rate-limit `OD-039`는 2026-08-25 승인됐다: LOGIN `900/5/100`, LOGOUT `60/10/300`, MFA_ENROLL `900/3/30`, MFA_VERIFY `300/5/100`, RECOVERY `3600/3/30`, REFRESH `60/10/500` (`seconds/subject/global`). 코드·DB는 canonical rule hash와 distinct `ADMIN_SECURITY` + `POSITION_LAB_DIRECTOR` Approval evidence를 요구하며, effective 승인 snapshot·실제 2인 actor·deployment HMAC secret 없이 운영 Auth mutation을 허용하지 않는다.
- Supabase Free 운영 대체경로는 `docs/supabase-free-operations.md`가 정본이다. 기존 R06 Identity Resolver가 provider `auth.sessions`에 대해 480분 최대시간, `refreshed_at` 기준 60분 refresh inactivity, newest-session-only를 매 요청 강제한다. 이는 마지막 화면 조작시각이 아니라 마지막 provider session refresh 시각 기준이다. 신규기기와 민감 action은 application-owned DeviceTrust/StepUpGrant가 필요하고, 15분 reconciliation·60분 encrypted Drive backup은 회사 통제 상시 Worker가 맡는다. Vercel Hobby Cron과 GitHub-hosted schedule은 승인 cadence의 단독 증거가 될 수 없다. Free 자동 pause/SLA 잔여위험은 production 승격 전 별도 승인·실증이 필요하다.
- 회원가입은 2026-08-25 P0에 추가됐다. 공개 `signUp`으로 Auth user/session을 선생성하지 않고 passwordless immutable 신청을 접수하며, direct active `POSITION_LAB_DIRECTOR` 본인만 단독 최종 승인/반려한다. 승인 후 server-only Worker가 Supabase invite를 실행하되 가입 승인은 Role/Position/VendorMembership/Scope를 부여하지 않는다. 현재 UserAccount 0명 상태에서 첫 연구소장을 만드는 one-time ceremony는 `OD-042`로 별도 release blocker이며 상세 정본은 `docs/identity-registration.md`다.
- `OD-042` 최초 연구소장 bootstrap ceremony와 `OD-043` 공개 가입 남용방지 초기값/provider는 2026-08-25 승인됐다. 먼저 restricted `ActivationContext`를 포함한 최소 DeviceTrust vertical slice를 구현한 뒤 bootstrap을 실행한다. OD-043은 email `3/24h`, HMAC network `10/h·30/24h`, global `100/h·500/24h`, always-on Turnstile, max-over-limit `Retry-After`, Vercel-owned client IP source와 deployment-specific hostname/action/sitekey·secret 결합을 사용한다. 실제 actor/runtime 승인 evidence가 적용되기 전 endpoint는 fail-closed한다.
