...
Issue `#58` B01 abuse-prevention slice는 login/recovery의 normalized identifier와 로그인 성공 시 발급한 256-bit HttpOnly stable limiter-subject를 분리한다. logout/TOTP enroll·verify/refresh는 token rotation과 무관한 limiter-subject를 공유하며 cookie가 없으면 Provider 호출 전 fail-closed한다. PostgreSQL은 subject+deployment-global fixed-window counter를 원자 소비하고 consume/result Audit 모두 exact policy UUID를 참조한다. canonical V1 hash가 정확히 6개 rule과 일치하고, distinct active `ADMIN_SECURITY` agreement + `POSITION_LAB_DIRECTOR` approval action을 가진 completed ApprovalInstance가 결합된 경우만 정책을 활성화한다. 수치·실제 2인 actor·HMAC secret은 임의 seed하지 않는다.
