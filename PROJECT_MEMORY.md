...
`IMPLEMENTATION_ACTIVE` (P0 `M00`~`M16`과 Release Gate #36 R01~R06 코드 계약이 `dev`에 병합 완료. `OD-019`, `OD-035`, `OD-036` 정책값은 2026-08-23 사용자 승인됐으나 실제 actor/stable-ID binding, effective approval snapshot, Staging 27개 증거와 `dev → main` 승격은 아직 없다. P1 범위·설계 Gate는 승인됐지만 P0 릴리즈와 별도 P1 Development Gate 전에는 제품 코드를 시작하지 않는다).
...
- 인증 rate-limit의 action별 window/subject/global 수치는 `OD-039`로 미확정이다. 코드·DB는 canonical rule hash와 distinct `ADMIN_SECURITY` + `POSITION_LAB_DIRECTOR` Approval evidence를 요구하며, 승인 snapshot·실제 2인 actor·deployment HMAC secret 없이 운영 Auth mutation을 허용하지 않는다.
