...
`IMPLEMENTATION_ACTIVE` (P0 `M00`~`M16`과 Release Gate #36 R01~R06 코드 계약이 `dev`에 병합 완료. `OD-019`, `OD-035`, `OD-036`과 `OD-039` 정책값, Primary/Pro/Vercel 및 monitoring/evidence/Google Drive backup stable ID는 사용자 승인됐으나 실제 actor binding, effective approval snapshot, Pro 전환, Staging 27개 증거와 `dev → main` 승격은 아직 없다. P1 범위·설계 Gate는 승인됐지만 P0 릴리즈와 별도 P1 Development Gate 전에는 제품 코드를 시작하지 않는다).
...
Supabase 연결 감사에서는 `ap-northeast-1`의 ACTIVE_HEALTHY 프로젝트 2개가 확인됐다. 사용자는 2026-08-25 `wrongstory's Project` (`dttwfqzkhjujqkcatyav`)를 `YOUONE_STAGING_PRIMARY`, 기존 `YOUONE_STAGING_RECOVERY` (`jzxhetszlucgutnwidkd`)를 격리 복구 대상으로 확정하고 Pro 전환을 승인했다. 현재 조직은 아직 Free이며 업무/Auth 데이터는 0건이고 Recovery는 migration/table이 비어 있다. 같은 날 Primary에 M11~M16, R02/R03/R06 및 B01 3개를 순서대로 적용해 hosted migration 21개가 됐다. 적용 후 public RLS 미적용 table과 `anon`/`authenticated`가 실행할 수 있는 public `SECURITY DEFINER`는 각각 0이며 Security Advisor는 WARN/ERROR 0, policy가 없는 FORCE-RLS table INFO만 남는다.
