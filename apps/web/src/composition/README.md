...
- Operational browser Auth also requires exact `APP_ORIGIN`; password/TOTP/refresh/recovery/logout provider calls remain inside `@youone/infra-supabase-auth/operational`. Production session/factor cookies are `Secure`, `HttpOnly`, `SameSite=Strict`, valid `__Host-` cookies and every mutation requires both the configured same origin and a double-submit CSRF value.
...
- `ACTIVATION_DATABASE_URL` identifies a separate least-privileged LOGIN that can set only `youone_activation`; it must not reuse the request or identity-resolver login. DeviceTrust composition additionally requires `DEVICE_TRUST_HMAC_SECRET` and the deployment-stable `ACTIVATION_PROVIDER_PROJECT_ID`. Production uses `ACTIVATION_DATABASE_TLS_MODE=verify-full`; no DeviceTrust policy duration is supplied through environment defaults.
