...
R06 requires approved versioned snapshots for `OD-019`, `OD-035` and `OD-036`. Each snapshot must be unrevoked and satisfy `createdAt <= approvedAt <= effectiveFrom <= evaluatedAt`; a pending/revoked approval, version mismatch, missing approver/evidence digest, null/placeholder value, future effective time, duplicate/unsupported value or backup cadence worse than the selected RPO fails closed. No production duration, owner, destination or provider credential is seeded in the repository.
