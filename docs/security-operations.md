...
Issue `#58` now adds the first operational browser/server Auth slice: same-origin plus double-submit CSRF, HttpOnly strict cookies, password login, TOTP enroll/verify, refresh, generic recovery request, global sign-out invocation and a current-session endpoint that reuses the trusted ActorContext chain. Provider credentials and factor/session identifiers are absent from response and fixed-field log schemas. This does not close the blocker: distributed rate limiting, recovery confirmation, device trust/step-up, actual Staging configuration and live evidence remain required. `docs/p0-operational-backend.md` is the detailed gap ledger.
...
The following B01 slice places an application-owned distributed rate-limit gate before login, logout, TOTP enroll/verify, refresh and recovery provider calls. PostgreSQL atomically consumes both subject and deployment-global buckets and appends the rate-limit decision in one request-principal transaction; a separate append-only record captures only the stable provider result class. The anonymous actor identity is a deployment-secret HMAC SHA-256 fingerprint. No email, password, access/refresh token, cookie, factor/session identifier, network address or provider body is persisted. Policy metadata and six normalized rules are immutable, revocation is append-only, tables are FORCE RLS with no Data API/request table grants, and only the guarded consume function is executable by `youone_request`. `OD-039` numeric values, approval snapshot/actor and deployment secret are intentionally absent, so operational Auth mutation remains fail-closed until they are explicitly provisioned and proven in Staging.

The connected primary candidate currently stops at hosted M10 and has direct managed-default `EXECUTE` grants from `anon`/`authenticated` to 124 public `SECURITY DEFINER` RPCs, producing 248 high-priority Advisor warnings plus five search-path warnings. B01 therefore includes a forward-fix that explicitly revokes both Data API roles, hardens future default privileges and asserts zero exposed public definers. The candidate is not production-ready and must not receive later migrations or #58 traffic until the environment identity is confirmed, the fix is applied in ordered migration rehearsal and Security Advisor returns no such warnings.
...
This closes the repository-code portion only. The production blocker remains open until two actual non-production Supabase projects complete the same no-public-object restore drill, the resulting evidence is retained, and the R05 Staging E2E gate accepts it. The user approved RPO 60 minutes, RTO 240 minutes, database backup every 60 minutes retained 14 days, and Storage backup every 60 minutes retained 30 days on 2026-08-23. Actual monitoring/evidence destinations, incident owner and disjoint recovery approver/executor sets remain unbound and no Staging drill has yet proved those objectives. Database restore evidence must include custom Web/Identity Resolver/Worker login password reprovisioning and subsequent readiness verification because provider database backups do not restore custom role passwords.
