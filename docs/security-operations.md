...
| `OD-019` policy values are approved but Free-plan application enforcement lacks Staging proof | Versioned policy snapshot and TOTP `aal2`, request-time 480m/60m/newest-session, DeviceTrust and StepUpGrant evidence |
| Supabase Free auto-pause and external scheduler residual risks are unproved | Company-controlled Worker cadence/heartbeat evidence, pause/resume runbook and explicit production residual-risk approval |
...
Supabase Free does not provide the approved sub-daily recovery objectives by itself. A company-controlled always-on Worker must execute the 15-minute reconciliation and 60-minute encrypted DB/Storage backup jobs. Vercel Hobby Cron is daily-only and GitHub-hosted scheduled workflows may be delayed or dropped, so neither is accepted as the sole RPO/cadence evidence. Free automatic project pausing is monitored and handled by an explicit resume/readiness runbook, never hidden by artificial keep-alive. The complete substitution and residual-risk contract is `docs/supabase-free-operations.md`.
