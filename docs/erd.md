...
B01 operational Auth adds immutable `AUTH_RATE_LIMIT_POLICY_VERSION`, six normalized `AUTH_RATE_LIMIT_POLICY_RULE` rows, append-only `AUTH_RATE_LIMIT_POLICY_REVOCATION`, and mutable `AUTH_RATE_LIMIT_BUCKET`. A policy snapshot binds approval hash, actual approving `UserAccount`, and created/approved/effective times; rules keep action/window/subject/global limits in typed columns rather than JSON. Bucket keys contain only `GLOBAL` or `SUBJECT` HMAC SHA-256 fingerprints, action and window counters. Tables use FORCE RLS with no request/Data API table policy; the request principal receives only the guarded atomic consume function. Each consume and provider result references a separate `AUTH_SECURITY_ATTEMPT` audit resource.
