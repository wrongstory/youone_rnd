...
M03 physically creates Identity/RBAC, Vendor/VendorUser, acting-authority, normalized action-set, and named field-projection records. P0 activation adds immutable `DEVICE_TRUST_POLICY_VERSION`/revocation, immutable `IDENTITY_ACTIVATION_EVIDENCE`/revocation and versioned `DEVICE_TRUST` with exact UserAccount, provider session, activation evidence and policy FKs. Only SHA-256 HMAC material is persisted; all tables use FORCE RLS and deny direct Data API/runtime-table access. `USER_REGISTRATION_APPLICATION`, immutable decision and provider provisioning remain approved P0 extensions for the following registration slice. `PROJECT_SCOPE`, `CONTRACT_SCOPE`, and exact DocumentVersion grant records remain logical extension points until M06, M07, and M05 can create them with their real typed FK targets and RLS in the same migration. M03 must not substitute a generic `(resource_type, resource_id)` grant table.
...
B01 operational Auth adds immutable `AUTH_RATE_LIMIT_POLICY_VERSION`, six normalized `AUTH_RATE_LIMIT_POLICY_RULE` rows, immutable `AUTH_RATE_LIMIT_POLICY_APPROVAL`, append-only `AUTH_RATE_LIMIT_POLICY_REVOCATION`, and mutable `AUTH_RATE_LIMIT_BUCKET`. The policy hash is recomputed from `YOUONE_AUTH_RATE_LIMIT_POLICY_V1`, policy stable version and six action-sorted `action|window|subject|max-global` lines. Approval links one completed `APPROVAL_INSTANCE` plus exact distinct `ADMIN_SECURITY` agreement and `POSITION_LAB_DIRECTOR` approval action IDs; assignment evidence and actor validity are rechecked at each action time. Bucket keys contain only `GLOBAL` or `SUBJECT` HMAC SHA-256 fingerprints. Tables use FORCE RLS with no request/Data API table policy. Consume and result `AUTH_SECURITY_ATTEMPT` audits both reference the exact policy UUID, and a result cannot append without its matching prior consume evidence.
