...
B01 abuse-prevention 경계는 Provider 호출 전에 `@youone/infra-postgres`의 request-principal capability로 subject/global fixed-window counter를 원자 소비하고 append-only Audit를 같은 transaction에 기록한다. Identifier·token·cookie·network 원문은 저장하지 않고 deployment-only HMAC secret으로 만든 SHA-256 fingerprint만 `ANONYMOUS` actor evidence로 남긴다. 정책은 immutable version/rule/revocation 관계이며 현재 effective·unrevoked version과 정확히 6개 action rule이 없으면 fail-closed한다. `anon`/`authenticated` 및 request role은 정책·bucket table을 직접 읽거나 쓸 수 없다. 실제 수치와 승인자 결합은 `OD-039` 전까지 미확정이므로 recovery confirmation, device trust/step-up, Worker reconciliation 및 Staging 증거와 함께 B01 blocker로 유지한다. 상세 현황은 `docs/p0-operational-backend.md`를 따른다.
...
R06 release boundary는 Worker composition의 fail-closed 검증기로 운영정책과 정확히 27개 증거를 검증한다. 정책은 `OD-019`/`OD-035`/`OD-036`의 exact decision/version, 승인자, `created <= approved <= effective <= evaluated`, 미철회 상태, raw artifact digest 및 명시값을 요구한다. 증거 경로는 입력에서 받지 않고 closed ID로 파생하며 실제 byte SHA-256을 재계산한다. 모든 evidence/R05 packet/trusted promotion source commit을 동일 SHA에 결합하고 R05 artifact를 다시 parse하여 environment, 5 readiness, required PASS check, artifact digest와 고정 canonical JSON digest를 재계산한다. `OD-036` runtime boundary는 request user ID 대신 trusted `USER_ACCOUNT`와 JWT `sub/session_id/issuer`, exact active session을 결합한 뒤 global sign-out과 세션 부재를 확인한다. 검증기는 deployment나 GitHub를 직접 변경하지 않고 내부 오류까지 `BLOCKED` 또는 `READY_FOR_RELEASE_PR` 두 상태로만 반환하므로 `main` 승격 권한은 별도 사용자 Gate에 남는다.
