...
| `OD-018-OFFLINE-MERGE` | Field-level merge rules per command and maximum offline data/security policy | `PARTIALLY_DECIDED` | `P0_SCHEMA_BLOCKER` for sync conflict payload and cache policy | Field scenarios + security review | Allowlist only; base-version conflicts never auto-apply |
...
| `OD-021-P0-MODULE-CUT` | Confirm optional P0/P1/P2 modules | `DECIDED` | none; Development Gate remains separately pending | User approved recommended selections as `P0-SCOPE-V1.0`, 2026-08-21 | P0: note/test/acceptance/NCR/CAR/ECR/ECO/safety-light/L3-L4 copy; remaining modules staged per checklist |
...
| `OD-027-PROJECT-STRUCTURE` | Approve `STRUCTURE-PROPOSAL-V1` pnpm workspace, package boundaries, route tree, and agent ownership | `PROPOSED` | `GATE_BLOCKER` before scaffold | Review `docs/project-structure.md` and `docs/agent-workstreams.md` | Do not create directories/dependencies until approved |
| `OD-028-DB-PRINCIPAL` | Choose the least-privileged request DB execution pattern: user JWT versus validated transaction-local actor context; isolate service role | `PROPOSED` | `P0_SCHEMA_BLOCKER` before RLS implementation | `ADR-004` PoC against Supabase/Postgres | Never inject service-role adapter into the normal request container |
| `OD-029-APPROVAL-SUBJECT` | Approve common ApprovalInstance plus typed subject-link tables/adapters for non-Document subjects | `PROPOSED` | `P0_SCHEMA_BLOCKER` before Approval migration | `ADR-003` and physical FK review | Do not use an unchecked generic subject ID or force all subjects into DocumentVersion |
