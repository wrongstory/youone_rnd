...
| `OD-039-AUTH-RATE-LIMIT-POLICY` | Select per-action fixed-window duration, subject limit and deployment-global limit for login, logout, TOTP enroll/verify, refresh and recovery | `WAITING_USER` | `RELEASE_BLOCKER` for operational Auth mutations | Security owner + Lab Director approval; bind the approved snapshot to an actual `UserAccount.id` | No numeric default is seeded. The Postgres limiter and audit contract exist, but an absent, incomplete, revoked, stale or non-current policy fails closed before Supabase provider dispatch. |
