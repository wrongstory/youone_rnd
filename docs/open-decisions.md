...
| `OD-035-PRODUCTION-OPERATIONS` | Approve production RPO/RTO, DB and Storage backup frequency/retention, monitoring destinations, incident owners and recovery approval authority | `PARTIALLY_DECIDED` | `RELEASE_BLOCKER` until actor/stable-ID binding, snapshot and drill evidence | User approved A values, 2026-08-23; actual operations actors/destinations pending | RPO 60m/RTO 240m, DB 60m/14d, Storage 60m/30d. Select actual incident/recovery approver/executor UUIDs and monitoring/evidence stable IDs; approver/executor sets must be disjoint and actual drill evidence must bind to the approved payload and finish within RTO. |
...
| `OD-039-AUTH-RATE-LIMIT-POLICY` | Select per-action fixed-window duration, subject limit and deployment-global limit for login, logout, TOTP enroll/verify, refresh and recovery | `WAITING_USER` | `RELEASE_BLOCKER` for operational Auth mutations | Distinct active `ADMIN_SECURITY` agreement + `POSITION_LAB_DIRECTOR` approval in one completed immutable ApprovalInstance; bind both actual `UserAccount.id` actors and exact actions | No numeric default is seeded. The canonical V1 SHA-256 must exactly match all six sorted rules. Missing/mismatched approval, hash, lifecycle, revocation or current-version evidence fails closed before Supabase provider dispatch. |
