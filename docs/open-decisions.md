...
| `OD-042-INITIAL-IDENTITY-BOOTSTRAP` | Approve the one-time creation and evidence ceremony for the first real Lab Director when no UserAccount exists | `OPEN` | `RELEASE_BLOCKER` for actual Staging actor creation and all actor-bound approvals | User / release owner | Recommended ceremony is in `docs/identity-bootstrap.md`: explicit user designation, separate executor/verifier, exact provider subject/UserAccount/Position assignment, TOTP/DeviceTrust, immutable evidence and immediate bootstrap-path removal. Do not add `FIRST_USER_IS_ADMIN`, email allowlists or self-approval. |
| `OD-043-REGISTRATION-ABUSE-POLICY` | Approve public registration request rate limits, CAPTCHA, network privacy and versioned evidence | `OPEN` | `FEATURE_BLOCKER` before B02-A public endpoint implementation/enabling | Security owner + direct Lab Director approval; user approves initial recommendation | `docs/registration-abuse-policy.md` recommends email `3/24h`, HMAC network `10/h` and `30/24h`, deployment global `100/h` and `500/24h`, one active request, always-on Turnstile, raw IP prohibition, exact `Retry-After` and fail-closed versioned policy. |
