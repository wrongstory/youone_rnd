...
| `OD-042-INITIAL-IDENTITY-BOOTSTRAP` | Approve the one-time creation and evidence ceremony for the first real Lab Director when no UserAccount exists | `OPEN` | `RELEASE_BLOCKER` for actual Staging actor creation and all actor-bound approvals | User / release owner | Do not add `FIRST_USER_IS_ADMIN`, email allowlists or self-approval. Bind the exact provider subject/UserAccount UUID, user approval evidence, executor/verifier, TOTP/DeviceTrust and audit import before ordinary registration opens. |
