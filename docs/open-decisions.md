...
| `OD-019-MFA-SESSION` | MFA requirement by actor/action, session duration, device controls, password/SSO policy | `WAITING_USER` | `RELEASE_BLOCKER` | User approval + security owner + Auth capability | R06 validates an approved version/evidence only; no default actor/action, duration or device value is seeded |
...
| `OD-035-PRODUCTION-OPERATIONS` | Approve production RPO/RTO, DB and Storage backup frequency/retention, monitoring destinations, incident owners and recovery approval authority | `WAITING_USER` | `RELEASE_BLOCKER` | User approval + management, security and operations owners + staging drill evidence | R06 enforces version/approval/evidence and backup cadence no worse than selected RPO; it does not seed intervals, owners or destinations. |
| `OD-036-SUPABASE-SESSION-REVOKE` | Select a supported non-destructive Supabase revoke-all-sessions-by-user operation for audited account disable workflow | `PROPOSED` | `RELEASE_BLOCKER` | User/security-owner approval + target-JWT handling and Staging capability proof | Official `global` sign-out requires a valid target-user JWT; access tokens can remain valid until expiry. Keep exact `auth.sessions` checks and fail closed until acquisition, residual-risk and retry/reconciliation policy are approved. |
| `OD-037-P1-DEVELOPMENT-GATE` | Confirm P1 module depth, cross-feature offline/notification/form scope, merge order and implementation start | `PARTIALLY_DECIDED` | `P1_DEVELOPMENT_GATE` | User approved recommended `P1-SCOPE-V1.0` and `DELIVERY-PLAN-P1-V0.1`, 2026-08-23; issue #48 owns proposed source/domain/ERD/permission/state/ADR review; P0 release and explicit implementation approval remain | Preserve the approved scope/order, but do not create P1 package/table/route/menu/migration until #36, #48 review and explicit implementation Gate are complete. |
