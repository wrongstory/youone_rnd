...
| `OD-040-SUPABASE-FREE-OPERATIONS` | Select the P0 operating substitute for Pro-only session controls and sub-daily backup/reconciliation scheduling | `PARTIALLY_DECIDED` | `RELEASE_BLOCKER` until application enforcement, external Worker evidence and Free residual-risk approval | User selected Supabase Free for now, 2026-08-25; step-up duration and production Free availability-risk acceptance remain pending | Keep Supabase Auth/TOTP/JWT, enforce 480m absolute / 60m `refreshed_at`-based refresh inactivity / newest-session in the Identity Resolver on every request, add application-owned DeviceTrust/StepUpGrant, and run 15m reconciliation plus 60m encrypted DB/Storage backup from a company-controlled Worker to private Google Drive. Do not use Vercel Hobby or GitHub-hosted schedule as sole cadence evidence and do not conceal Free auto-pause with artificial keep-alive. |
| `OD-041-USER-REGISTRATION-APPROVAL` | Select the signup creation boundary and final approval authority | `DECIDED` | `RELEASE_BLOCKER` until registration policy, provider invite and Staging evidence are effective | User selected Lab Director direct approval, 2026-08-25 | Public request creates no Supabase user/password/session. A direct active `POSITION_LAB_DIRECTOR` actor alone approves or rejects the immutable request; no Senior/Representative/Admin/acting-authority substitute. Approval queues a server-only provider invite but grants no Role, Position, VendorMembership or Scope. |
| `OD-042-INITIAL-IDENTITY-BOOTSTRAP` | Approve the one-time creation and evidence ceremony for the first real Lab Director when no UserAccount exists | `DECIDED` | `RELEASE_BLOCKER` until minimal DeviceTrust/ActivationContext exists and the actual Staging ceremony evidence passes | User approved the corrected ceremony, 2026-08-25; actual target/executor/verifier remain operational inputs | Ceremony in `docs/identity-bootstrap.md`: DeviceTrust vertical slice first, restricted PENDING-only ActivationContext, explicit target designation, separate executor/verifier, exact provider subject/UserAccount/Position, TOTP/DeviceTrust, immutable evidence and immediate bootstrap-path removal. No `FIRST_USER_IS_ADMIN`, email allowlist or self-approval. |
| `OD-043-REGISTRATION-ABUSE-POLICY` | Approve public registration request rate limits, CAPTCHA, network privacy and versioned evidence | `DECIDED` | `FEATURE_BLOCKER` until the approved policy and two-person runtime evidence are effective | User approved initial values/provider, 2026-08-25; effective runtime policy still needs distinct active Security agreement + Lab Director approval | Email `3/24h`, HMAC network `10/h` and `30/24h`, deployment global `100/h` and `500/24h`, one active request, always-on Turnstile, Vercel-owned client-IP source, no raw IP or Siteverify `remoteip`, max-over-limit `Retry-After`, deployment-specific sitekey/secret + hostname/action binding and fail-closed versioned policy. |
