...
`ActivationContext` is a separate transient trusted value, never a Role or partial business ActorContext. It is re-derived from an exact live provider session, PENDING UserAccount, verified TOTP `aal2` and approved invite/bootstrap activation evidence, and authorizes only exact-self DeviceTrust enrollment/verification plus minimal readiness query. It grants no general repository/Data API access. Completing DeviceTrust makes another enrollment a replay but does not remove verification/readiness while the account is still PENDING; the context ceases when the account becomes ACTIVE or the session, TOTP, account validity or source evidence is no longer valid.

`DeviceTrustPolicyVersion` is an immutable, approval-bound duration/method snapshot. No current version or maximum duration is seeded; absence, digest mismatch, revocation or non-effective time fails closed. `IdentityActivationEvidence` immutably binds one PENDING UserAccount to either a registration approval or `OD-042` bootstrap evidence, exact provider subject, invitation/password completion and validity. `DeviceTrust` stores only the one-way credential HMAC with exact UserAccount/provider session/evidence/policy, typed `PENDING | ACTIVE | REVOKED | EXPIRED` state, UTC lifecycle timestamps and optimistic version. Raw nonce, browser fingerprint, cookie and provider token are not domain fields.
