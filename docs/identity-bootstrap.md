...
본 ceremony 정책은 2026-08-25 사용자 승인으로 확정됐다. 실제 대상자, 실행자, 검증자와 실행시각은 별도 운영 입력이며 evidence 결합 전까지 확정값으로 취급하지 않는다. 실제 bootstrap 실행은 최소 DeviceTrust vertical slice와 아래 `ActivationContext`가 배포·검증된 뒤에만 허용한다.
...
1. 최소 DeviceTrust vertical slice와 restricted `ActivationContext`를 배포하고 PENDING 계정의 업무 접근 거부/DeviceTrust enrollment 허용을 검증한다.
2. exact Staging project, candidate commit, migration head와 Security Advisor 상태를 캡처한다.
3. server-only Admin invite로 대상 provider user를 만들고 반환된 exact `auth.users.id`를 확인한다.
4. PENDING `UserAccount`를 만들고 provider subject를 exact 결합한다.
5. 별도 audited 명령으로 유효한 `POSITION_LAB_DIRECTOR` assignment를 만든다. Role이나 다른 Position은 자동 부여하지 않는다.
6. 대상자가 초대를 수락하고 password 설정과 verified TOTP `aal2`를 완료한다.
7. 서버가 아래 제한된 `ActivationContext`를 파생하고 해당 context로만 DeviceTrust를 등록한다.
8. Identity Resolver가 exact provider `session_id`, PENDING/ACTIVE 상태, PositionAssignment와 DeviceTrust를 재검증한다.
9. 모든 활성조건이 충족된 한 transaction에서 UserAccount를 `ACTIVE`로 전이하고 append-only Audit/outbox를 기록한다.
10. 독립 검증자가 provider subject ↔ UserAccount ↔ PositionAssignment ↔ TOTP ↔ DeviceTrust ↔ Audit evidence를 대조한다.
11. bootstrap용 임시 접근·명령·credential을 회수하고 같은 경로의 재실행이 fail-closed하는지 확인한다.

초대 수락으로 Supabase session이 생겨도 `UserAccount=PENDING` 동안 일반 업무 `ActorContext`를 만들지 않는다.

### ActivationContext

`ActivationContext`는 일반 `ActorContext`의 축소판이나 Role이 아니라 서버가 요청마다 파생하는 별도 trusted context다. 다음 조건을 모두 만족할 때만 만든다.

```text
exact provider issuer/project/sub/session_id
AND live auth.sessions row
AND exact PENDING UserAccount
AND verified TOTP aal2
AND exact approved invite/bootstrap activation evidence
AND not disabled/revoked/expired
────────────────────────────────────────────
ActivationContext
```
