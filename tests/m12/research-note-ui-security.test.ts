...
describe("M12 ResearchNote UI/API security", () => {
  ...
  it("shows one optional Senior-review path and one finalized skip/correction chain", async () => {
    ...
    await expect(researchNoteQuery(true).getInternal(PREVIEW_IDS.researchNoteFinalized)).resolves.toMatchObject({ availability: "AVAILABLE", detail: { state: "FINALIZED", seniorReviewPath: "SKIPPED_BY_POLICY", directorFinalization: { representativeApprovalIncluded: false }, correctionChain: [{ kind: "CORRECTION" }, { kind: "ADDENDUM" }], pdfEvidence: { deliveryRestriction: "ORIGINAL_DENIED" } } });
  });
