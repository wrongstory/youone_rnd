...
describe("M11 Purchase/R&D migration contract", () => {
  ...
  it("forces RLS, revokes request writes, hard-denies HQ mutation and exposes explicit projections", () => {
    ...
    expect(sql).toContain("function app_private.m11_actor_can_read_rnd");
    expect(sql).toContain("grant execute on function app_private.m11_actor_is_hq(timestamptz),app_private.m11_actor_can_read_rnd(timestamptz) to youone_request");
